...
| Provider Save is disabled on Vercel | Vercel uses environment-managed provider settings because `/var/task` is read-only | Add the provider's environment variable in Vercel Project Settings, then redeploy; the UI can test a key but does not store it |
