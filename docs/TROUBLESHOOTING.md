...
| No AI generation | No usable provider/model is configured, the provider test failed, or the request exceeded the LLM timeout | Open **Settings → Providers**, choose a preset or OpenAI-compatible endpoint, enter your own key/model, click **Test**, then **Save**. `LLM_TIMEOUT` is retryable; choose a faster model or adjust `AGENTSCAD_LLM_TIMEOUT_MS`. Templates are disabled in the normal product path. |
