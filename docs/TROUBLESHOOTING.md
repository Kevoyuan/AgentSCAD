...
| No AI generation | No usable provider/model is configured, the provider test failed, or the provider/hosting platform ended the request | Open **Settings → Providers**, choose a preset or OpenAI-compatible endpoint, enter your own key/model, click **Test**, then **Save**. AgentSCAD has no model timeout by default; `AGENTSCAD_LLM_TIMEOUT_MS` is an opt-in limit. Hosting platforms may still enforce a function duration limit. Templates are disabled in the normal product path. |
